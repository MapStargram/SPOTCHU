...
// ⚠️ "로그인 후 연결하기"만 안내하면 안 됨: 지금 로그인된 계정에서 연결을 시도하다 이 에러가 났다면,
//   그 소셜이 물려있는 다른 계정으로 가서 반대로 연결해도 이번엔 지금 계정 쪽 소셜이 이미 남에게 물려있어
//   똑같이 막히는 순환이 될 수 있다(두 계정이 서로 다른 소셜을 하나씩 이미 갖고 있는 경우).
//   그 경우 실제로 통하는 유일한 길은 "그 계정에 비밀번호를 먼저 설정 → 소셜 해제 → 지금 계정에서 재연결"
//   뿐이라(lib/actions/auth.ts의 disconnectProvider/setPassword 조합), 안내에 이 경로까지 포함한다.
function authErrorMessage(code: string): string {
  switch (code) {
    case "OAuthAccountNotLinked":
      return "이 소셜은 이미 다른 계정에 연결돼 있어요. 그 계정으로 로그인해서 연결해 주세요. 거기서도 막힌다면, 그 계정에 비밀번호를 먼저 설정한 뒤 소셜 연결을 해제하고 지금 계정에서 다시 연결하면 돼요.";
    default:
