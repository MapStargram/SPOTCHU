...
- **회원탈퇴(계정 삭제)**: 설정 > **위험 구역**에서 결과 안내 + "되돌릴 수 없음" 동의 체크 후 `deleteAccountAction`(`lib/actions/auth.ts`) 실행. 정책 = **개인데이터 삭제 + 제보 스팟 익명화**:
  ...
  - **스토리지 정리**: 게시물 사진·아바타의 Cloudinary 원본을 DB 삭제 후 **best-effort로 삭제**(우리 클라우드 URL만 — 소셜 제공 아바타는 대상 아님; Cloudinary 실패가 탈퇴를 막지 않음). 같은 정리는 운영자 게시물/사진 삭제(`admin.ts`)에도 적용.
...
- Cloudinary 정리는 **best-effort** — 삭제 실패한 고아 자산의 주기적 배치 정리(reconcile)는 후속. 삭제된 유저의 JWT 토큰 즉시 거부(현재는 클라 `signOut` + DB 조회 게스트 폴백)도 후속.
